...
spec = do
  describe "encodePunycode" $ do
    it "encodes Arabic (Egyptian) domain" $ do
      encodePunycode "مصر" `shouldSatisfy` isRight
    
    it "encodes Chinese domain" $ do
      encodePunycode "中国" `shouldSatisfy` isRight
    
    it "encodes German domain with umlaut" $ do
      let result = encodePunycode "münchen"
      result `shouldBe` Right "mnchen-3ya"
    
    it "encodes Russian domain" $ do
      encodePunycode "испытание" `shouldSatisfy` isRight
    
    it "encodes Japanese (Hiragana) domain" $ do
      encodePunycode "ひらがな" `shouldSatisfy` isRight
    
    it "encodes Japanese (Katakana) domain" $ do
      encodePunycode "カタカナ" `shouldSatisfy` isRight
    
    it "encodes Korean domain" $ do
      encodePunycode "한국" `shouldSatisfy` isRight
    
    it "encodes Hindi domain" $ do
      encodePunycode "भारत" `shouldSatisfy` isRight
    
    it "encodes Greek domain" $ do
      encodePunycode "ελλάδα" `shouldSatisfy` isRight
    
    it "encodes Thai domain" $ do
      encodePunycode "ไทย" `shouldSatisfy` isRight
    
    it "encodes mixed ASCII and Unicode" $ do
      encodePunycode "hello世界" `shouldSatisfy` isRight
    ...
    it "encodes emoji domains" $ do
      encodePunycode "😀" `shouldSatisfy` isRight
  ...
  describe "decodePunycode" $ do
    it "decodes German domain" $ do
      decodePunycode "mnchen-3ya" `shouldBe` Right "münchen"
    ...
    it "decodes Chinese domain" $ do
      let result = decodePunycode "fiq228c"  -- 中国
      result `shouldSatisfy` isRight
    
    it "decodes Arabic domain" $ do
      let result = decodePunycode "wgbl6c"  -- مصر
      result `shouldSatisfy` isRight
    ...
    it "rejects non-basic characters before delimiter" $ do
      decodePunycode "münchen-test" `shouldSatisfy` isLeft
